...
public class IntercepterAdmin extends HandlerInterceptorAdapter {
    ...
    public boolean preHandle(HttpServletRequest request,
        ...
        //获取所有的权限
        List<SysFunction> allFunctionList = (List<SysFunction>) CacheUtil.get(CacheConstans.SYS_ALL_USER_FUNCTION_PREFIX);
        if (ObjectUtils.isNull(allFunctionList)) {
            allFunctionList = sysFunctionService.queryAllSysFunction();
            CacheUtil.set(CacheConstans.SYS_ALL_USER_FUNCTION_PREFIX, allFunctionList);
        }
        //判断当前访问的权限，是否在限制中
        boolean hasFunction = false;
        for (SysFunction sf : allFunctionList) {
            if (sf.getFunctionUrl() != null && sf.getFunctionUrl().trim().length() > 0 && sf.getFunctionUrl().indexOf(invokeUrl) != -1) {
                hasFunction = true;
                break;
            }
        }
        ...
        if (ObjectUtils.isNotNull(userFunctionList)) {
            for (SysFunction usf : userFunctionList) {
                //如果用户有访问权限
                if (usf != null && invokeUrl != null && usf.getFunctionUrl() != null && usf.getFunctionUrl().indexOf(invokeUrl) != -1 && usf.getFunctionUrl().trim().length() > 0) {
                    flag = true;
                    break;
                }
            }
        ...
        if (!flag) {
            if (WebUtils.isAjaxRequest(request)) {
                ...
                response.getWriter().print(FastJsonUtil.obj2JsonString(json));
            } else {
