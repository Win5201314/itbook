...
public class AdminEmailController extends BaseController{
	...
	public String querySendEmailMsgList(Model model,@ModelAttribute("userEmailMsg") UserEmailMsg userEmailMsg, @ModelAttribute("page") PageEntity page) {
		try {
			page.setPageSize(10);
	...
	@SystemLog(type="del",operation="删除发送邮件记录")
	public String delSendEmailMsg( @RequestParam("id") Long id) {
		try {
	...
	public Map<String, Object> importExcel(HttpServletRequest request, @RequestParam("myFile") MultipartFile myFile, @PathVariable("type") int type) {
		...
		try {
			...
			// 获得页面的
			if (StringUtils.isNotEmpty(request.getParameter("numerStr"))) {
	...
	@ResponseBody
	@SystemLog(type="add",operation="发送邮件")
	...
	@ResponseBody
	@SystemLog(type="update",operation="修改未发送的邮件")
