...
public class CommentServiceImpl implements CommentService {
	...
	public void addComment(Comment comment) {
		...
		comment.setContent(new SensitivewordFilter(sensitiveWordsService).replaceSensitiveWord(comment.getContent(), 1, "*"));
	 ...
	 * 查询最近10条讨论
	 */
	public List<Comment> queryCommentTalk(Comment comment){
		return commentDao.queryCommentTalk(comment);
	}

	@Override
	public List<Comment> queryCourseComment(Comment comment, PageEntity page) {
		return commentDao.queryCourseComment( comment, page);
	}
}
