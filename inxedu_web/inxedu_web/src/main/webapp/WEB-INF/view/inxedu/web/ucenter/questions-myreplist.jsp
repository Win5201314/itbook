...
<body>
	<article class="col-80 fl">
		<div class="u-r-cont">
			<section>
				<div>
					<span class="fr"><a href="${ctx }/questions/toadd" title="" class="comm-btn c-btn-6 tw-btn">去提问</a></span>
					<section class="c-infor-tabTitle c-tab-title cnew-tab-title">
						<a href="${ctx }javascript: void(0)" title="Wo的问答" class="c-tab-title">Wo的问答</a>
											...
											<aside class="q-head-pic">
												<c:choose>
													<c:when test="${not empty question.picImg }">
														<img src="<%=contextPath %>${question.picImg }" alt="">
													</c:when>
													<c:otherwise>
												...
												<p class="hLh30 txtOf">
													<span class="c-999">
														<c:if test="${not empty question.displayName }">${question.displayName }</c:if>
													</span>
												</p>
											</aside>
