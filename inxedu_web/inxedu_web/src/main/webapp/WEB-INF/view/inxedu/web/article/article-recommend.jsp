...
<div class="ml30 artil-list-box bg-fff">
				...
				<c:forEach var="article" items="${articleList }" varStatus="index">
					<li>
						...
						<h3 class="hLh30 txtOf">
							<a href="${ctx }/front/articleinfo/${article.articleId}" title="${article.title }" class="i-art-title">${article.title }</a>
						</h3>
						...
						<%--<section class="hLh30 txtOf mt5">
							<div class="fr">
								<span> <a class="noter-dy vam" title="回答" href="${ctx }/front/articleinfo/${article.articleId}">
	...
	<%--<section style="margin-bottom: 20px;">
		<c:forEach var="image" items="${websiteImages.type_12}" varStatus="status">
			<c:if test="${status.index==0 }">
				<a href="${image.linkAddress}" title="${image.title}"><img src="${staticServer}${image.imagesUrl}" width="100%" alt="${image.title}"/></a>
			</c:if>
		</c:forEach>
	</section>--%>
</div>
