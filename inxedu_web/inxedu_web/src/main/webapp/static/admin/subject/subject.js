...
function addSubject() {
	...
	if (subjectParentId != null) {
        layer.msg("专业最多只支持两级", {icon: 5, shift: 6});
		return;
	}
	$.ajax({
...
function setIframeHeight() {
	var parentIframeIdval=$("#parentIframeId").val();
	var resizeHeight = $('#'+parentIframeIdval, parent.document).height();
	var uiSMenuHeight=$('#ui-sMenu', parent.document).height();
	/*layer.msg(resizeHeight, {icon: 5, shift: 6});
	 layer.msg(uiSMenuHeight, {icon: 5, shift: 6});*/
	if(uiSMenuHeight<resizeHeight){
		$('#'+parentIframeIdval, parent.document).height(resizeHeight);
		$('#'+parentIframeIdval, parent.document).parent().parent().height(resizeHeight);
	}else{
		$('#'+parentIframeIdval, parent.document).height(uiSMenuHeight);
		$('#'+parentIframeIdval, parent.document).parent().parent().height(uiSMenuHeight);
	}
}
