...
var vP = function() {
	...
	$(".n-p-v-b-rt-in").css("height", wH - 100);
	$(".p-h-r-list .lh-menu").css("height", wH - 160);
	$(".playBox").css("height", wH - 100);
	//讨论动态高度赋值
	$("#p-h-r-cont .p-center-discuss,#p-h-r-cont .talkhtml").height($("#p-h-r-cont").height()-110);
};
// 右侧菜单区域显示与隐藏
var rmFun = function() {
	$("#o-c-btn a").click(function() {
		var _this = $(this);
		if (!_this.hasClass("close")) {
			$(".n-p-v-b-rt").animate({"right" : "-380px"}, 400);
			$(".n-p-vb-in").animate({"padding-right" : "0"}, 400);
			_this.parent(".video-open-btn").animate({right:"-15px"},400);
			_this.addClass("close").attr("title", "收起");
		} else {
			$(".n-p-v-b-rt").animate({"right" : "0"}, 500);
			$(".n-p-vb-in").animate({"padding-right" : "380px"}, 400);
			_this.parent(".video-open-btn").animate({right:"365px"},400);
			_this.removeClass("close").attr("title", "展开");
		}
	})
};
// 开关灯效果
