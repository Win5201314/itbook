...
<body>
	<div class="">
		<form action="${ctx}/admin/commentreply/query" method="post" id="searchForm">
			...
			<input placeholder="昵称/邮箱/手机" type="text" name="keyWord" value="${comment.keyWord}" />
			<input placeholder="内容" type="text" name="content" value="${comment.content}" />
			<a onclick="$('#searchForm').submit();" class="layui-btn layui-btn-small" href="javascript:void(0)">查找评论
			</a>
			<%--<a title="清空" onclick="$('#searchForm input:text').val('');$('#searchForm select').val(0);" class="layui-btn layui-btn-small"
