...
public class CourseStudyhistoryController extends BaseController{
	...
	public Object addCoursePlayerTime(HttpServletRequest request, @ModelAttribute("courseId") Long courseId,
		...
		try {
			...
			if(StringUtils.isNotEmpty(request.getParameter("playTime"))){
				courseStudyhistory.setDataback(request.getParameter("playTime")+"|"+request.getParameter("totalTime"));
			}
	...
	public Object getCourseLearnedUser(HttpServletRequest request, @PathVariable("courseId") Long courseId) {
		...
		try {
			//从缓存中获取
			//学习此课程的总人数
			Long couStudyhistorysLearnedCount=(Long)CacheUtil.get("COURSE_LEARNED_USER_COUNT_"+courseId);
			//学习此课程的人 (最新8个)
			...
			if(couStudyhistorysLearned==null||couStudyhistorysLearned.size()==0){
				...
				CacheUtil.set("COURSE_LEARNED_USER_COUNT_"+courseId, couStudyhistorysLearnedCount,300);//缓存5分钟
			}
