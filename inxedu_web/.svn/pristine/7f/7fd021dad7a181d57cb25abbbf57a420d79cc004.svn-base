...
$.extend($.fn, {
    initJerichoTab: function(setting) {
        ...
        function createJerichoTab() {
            ...
            */
            var jerichotab = $('<div id="'+opts.uniqueId+'" class="jericho_tab"><div class="tab_pages" ><div class="tabs"><ul /></div></div><div class="tab_content"><div id="jerichotab_contentholder" class="content" /></div></div></div>')
            ...
            $.fn.jerichoTab = {
                ...
                addTab: function(tabsetting) {
                    ...
                    $("#jerichotab_contentholder").attr("style","height:"+$("#jerichotab_contentholder").height()+"px");
                    ...
                    if (ps.tabFirer != null) {
                        ...
                        if (typeof jerichotabindex != 'undefined' && $('#jerichotab_' + jerichotabindex).length > 0){
                            //$('#jerichotab_' + jerichotabindex).remove();
    ...
    buildIFrame: function(src) {
        return this.each(function() {
            ...
            var iframe = $('<iframe id="' + jerichotabiframe + '" name="' + jerichotabiframe + '" src="' + src + '" frameborder="0" scrolling="auto" />')
							.css({ width: '100%', border: 0 }).appendTo($(this));
            ...
            //iframe 加载完成后 设置mainFrame 高度
            var tabiframe=document.getElementById(jerichotabiframe);

            tabiframe.onload = function(){
                var iframeWin = tabiframe.contentWindow || tabiframe.contentDocument.parentWindow;
                if (iframeWin.document.body) {
                    ...
                    var windowHeight=$(window).height()-$("#header").height()-$("#footer").height()-130;
                    if((uiSMenuHeight<windowHeight && resizeHeight<windowHeight)){
                        tabiframe.height = windowHeight;
                        $("#"+jerichotabiframe).parent().parent().css("height",windowHeight);
                    }
                    else if(uiSMenuHeight<resizeHeight){
                        tabiframe.height = resizeHeight;
                        $("#"+jerichotabiframe).parent().parent().css("height",resizeHeight);
                    }else{
                        tabiframe.height=uiSMenuHeight;
                        $("#"+jerichotabiframe).parent().parent().css("height",uiSMenuHeight);
                    }

                    $("#"+jerichotabiframe).css("visibility","visible");

                    //把当前 iframeid 传给子页面
                    $(window.frames[jerichotabiframe].document).find("input[id='parentIframeId']").val(jerichotabiframe);
                }
            };
        });
    },
    ...
    loadData: function(flag) {
        return this.each(function() {
        	/*$('.jericho_tab .tab_selected').css({'background-color':'#616470','color':'#fff'});*/
            ...
            function load(c) {
                switch (dataType) {
                    ...
                    case 'iframe':
                        ...
                        holder.buildIFrame(dataLink, function() {
                            removeLoading(holder);
                        });
                        break;
                    case 'ajax':
