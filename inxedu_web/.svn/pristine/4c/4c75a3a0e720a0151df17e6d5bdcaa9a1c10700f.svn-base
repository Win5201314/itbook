...
public class SysFunctionController extends BaseController{
	...
	public ModelAndView showSysFunctonZtree(HttpServletRequest request){
		...
		try{
			...
			if(functionList!=null){
				model.addObject("jsonFunction", FastJsonUtil.obj2JsonString(functionList));
			}
		}catch (Exception e) {
	...
	@ResponseBody
	@SystemLog(type="add",operation="添加系统权限")
	public Map<String,Object> addFunction(HttpServletRequest request,@ModelAttribute("sysFunction") SysFunction sysFunction){
		...
		try{
			...
			json = this.setJson(true, null, FastJsonUtil.obj2JsonString(sysFunction));
		}catch (Exception e) {
	...
	@ResponseBody
	@SystemLog(type="update",operation="修改系统权限")
	public Map<String,Object> udpateFunction(HttpServletRequest request,@ModelAttribute("sysFunction") SysFunction sysFunction){
		Map<String,Object> json = new HashMap<String,Object>();
		try{
			...
			CacheUtil.remove(CacheConstans.USER_FUNCTION_PREFIX+sysuser.getUserId());
	...
	@ResponseBody
	@SystemLog(type="del",operation="删除系统权限")
	public Map<String,Object> deleteFunction(HttpServletRequest request,@PathVariable("ids") String ids){
		Map<String,Object> json = new HashMap<String,Object>();
		try{
			sysFunctionService.deleteFunctionByIds(ids);
			//清除 用户权限 缓存，让用户刷新重新加载
			CacheUtil.remove(CacheConstans.USER_FUNCTION_PREFIX + SingletonLoginUtils.getLoginSysUserId(request));
