...
public class LoginController extends BaseController{
	...
	public String outLogin(HttpServletRequest request,HttpServletResponse response){
		try{
			...
			//删除所有的权限缓存
			CacheUtil.remove(CacheConstans.SYS_ALL_USER_FUNCTION_PREFIX);
			//删除登录用户的缓存信息
	...
	public ModelAndView login(HttpServletRequest request,HttpServletResponse response,@ModelAttribute("sysUser") SysUser sysUser){
		...
		try{
			...
			//缓存用登录信息
			CacheUtil.set(CacheConstans.LOGIN_MEMCACHE_PREFIX+su.getUserId(), su,86400);//缓存一天
			...
			//删除登录用户权限缓存
			CacheUtil.remove(CacheConstans.USER_FUNCTION_PREFIX+su.getUserId());
			//删除所有的权限缓存
			CacheUtil.remove(CacheConstans.SYS_ALL_USER_FUNCTION_PREFIX);
		}catch (Exception e) {
