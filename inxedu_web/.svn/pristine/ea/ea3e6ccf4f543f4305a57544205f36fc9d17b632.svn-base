		...
		<section>
			<div>
				<section class="c-infor-tabTitle c-tab-title">
					...
					<a href="${ctx }/uc/freeLiveList" title="免费" class="current">免费</a>
					<a href="${ctx }/uc/overduelive" title="已过期">已过期</a>
				</section>
			</div>
			<div class="mt40">
								...
								<li <c:if test="${course.liveBeginTime.time<nowDate and course.liveEndTime.time>nowDate}">class="current"</c:if> >
									<div class="live-cb-wrap">
										...
										<div class="live-cou-time clearfix">
											...
											<div class="fr">
												<a href="javascript:void(0)" onclick="nowLivePlay(${course.courseId})">
													<c:if test="${nowDate-course.liveEndTime.time < 0}">进入直播</c:if>
													<c:if test="${nowDate-course.liveEndTime.time > 0}">查看回放</c:if>
												</a>
											</div>
										</div>
									</div>
				...
				<!-- 公共分页 结束 -->
				<form method="post" id="searchForm" action="${ctx}/uc/freeCourseList?sellType=${queryCourse.sellType}">
