...
<script type="text/javascript">
	...
	function update(navId,obj) {
		...
		$("textarea[name='websiteNavigate.image']").val($(obj).parent().parent().find(".navigateImage").html());
    ...
    function addNavigates() {
		...
		$("input[name='websiteNavigate.image']").val("");
						...
						<c:if test="${websiteNavigates.size()>0}">
							<c:forEach items="${websiteNavigates}" var="navigate" varStatus="index">
								<tr <c:if test="${index.count%2==1 }">class="odd"</c:if>>
									...
									<td class="navigateImage">${navigate.image }</td>
	...
	<div id="updateNaveInfo" style="display: none">
		<div class="numb-box">
			<form class="layui-form" action="${ctx}/admin/website/updateNav">
				...
				<div class="layui-form-item">
					<label class="layui-form-label layui-form-label-w">导航svg图片代码</label>
					<div class="layui-input-block">
						<textarea  required="required"  name="websiteNavigate.image" value=""  class="layui-input layui-input-6"></textarea>
					</div>
				</div>
				<div class="layui-form-item">
				...
				<input type="hidden" name="websiteNavigate.type" value="FOOTER"/>
