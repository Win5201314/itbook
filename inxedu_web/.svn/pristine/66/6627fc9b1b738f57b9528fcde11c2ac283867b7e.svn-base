...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="${ctx}/static/common/jquery-ui-1.10.4/css/ui-lightness/jquery-ui-timepicker-addon.css?v=${v}" />
    ...
    function query(){
        $.ajax({
            ...
            success:function (result){
                ...
                var returnMes = result.entity.returnMes;
                var obj = eval("("+returnMes+")");
                if (returnMes.result!=0){
                    $("#status").html(obj.errmsg);
                    return;
                }
                if(listNum==0){
                    $("#num").html("");
           ...
           <!--  <caption>&nbsp;</caption>
            ...
            <tr>
                <td colspan="3" align="center">
                    <c:if test="${type==1}">
                    	<input onclick="window.location.href='${ctx}/admin/email/sendEmaillist'" class="layui-btn layui-btn-small" type="button" value="返回" />
                    </c:if>
                    <c:if test="${type==2}">
                    	<input onclick="window.location.href='${ctx}/admin/mobile/sendMsglist'" class="layui-btn layui-btn-small" type="button" value="返回" />
                    </c:if>
                </td>
            </tr>
            <!-- </tbody> -->
