...
<body>
	<div id="container">
		<div class="logo">
			<a href="${ctx}" target="_blank" title="登录 -${websitemap.web.company}-${websitemap.web.title}">
				<img src="${websitemap.web.logo}" width="200" alt="${websitemap.web.company}-${websitemap.web.title}" />
			</a>
		</div>
	</div>
	...
	<div class="b-box">
		<div id="container">
			<div id="box">
				...
				<form action="${ctx}/admin/main/login" method="POST" id="loginForm">
					<p class="main">
						...
						<label>密码: </label>
						<input type="password" onkeyup="enterSubmit(event)" name="sysUser.loginPwd" placeholder="输入密码">
					</p>
					<p class="main">
						<label>验证码: </label>
						<input name="randomCode" onkeyup="enterSubmit(event)" placeholder="验证码" style="width: 105px;"  maxlength="4"/>
						<span class="yzm-pic">
							<img src="${ctx}/ran/ajax/random" alt="验证码，点击图片更换" onclick="this.src='${ctx}/ran/ajax/random?random='+Math.random();" />
						</span>
					</p>
					<p class="space">
			...
			<div class="login-foot">
				<span>
					Powered By <a target="_blank" href="${ctx}" style="color: #666;">${websitemap.web.company}</a>
				</span>
			</div>
		</div>
	</div>
