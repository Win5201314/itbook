...
<script type="text/javascript">
	function addSubmit(){
		if($("#navigateName").val()==null||$("#navigateName").val()==""){
            layer.msg("名称不能为空", {icon: 5, shift: 6});
			return false;
		}
		if($("#navigateUrl").val()==null||$("#navigateUrl").val()==""){
            layer.msg("跳转链接不能为空", {icon: 5, shift: 6});
			return false;
		}
		if(isNaN($("#orderNum").val())){
            layer.msg("排序只能为数字", {icon: 5, shift: 6});
			return false;
		}
	...
	<div class="numb-box"><fieldset class="layui-elem-field">
		...
		<div class="mt20">
			<form action="${ctx}/admin/website/addNavigate" method="post" id="addNavigateForm">
				...
				<p>
					<input type="button" value="提 交" class="layui-btn layui-btn-small" onclick="addSubmit()" />
					<input type="reset" value="返 回" class="layui-btn layui-btn-small" onclick="history.go(-1);" />
				</p>
			</form>
		</div>
	</fieldset>
